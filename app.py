...
import base64
import time
import random # تم إضافة هذه المكتبة لاختيار مفتاح عشوائي والتهرب من الضغط
...
# 2. تفعيل مفاتيح جوجل الثلاثة (التعديل الجديد لتوزيع الأحمال)
try:
    keys = [st.secrets["API_KEY_1"], st.secrets["API_KEY_2"], st.secrets["API_KEY_3"]]
...
with col_img2:
    if os.path.exists("waw_logo.png"):
        st.image("waw_logo.png", use_container_width=True)
                ...
                try:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            if "429" in str(inner_e).lower() or "quota" in str(inner_e).lower():
                                if attempt < max_retries - 1:
                                    time.sleep(4) # انتظار 4 ثوانٍ في الخلفية ثم إعادة المحاولة
                                    continue
                            raise inner_e # رفع الخطأ إذا لم يكن بسبب الضغط أو انتهت المحاولات
                    ...
                    # التعديل الجديد لاصطياد وإظهار الخطأ فوراً
    ...
    with lib_tab1:
        ...
        with st.expander("🧩 اضطراب طيف التوحد (الدمج الخفيف)"):
            ...
            * **إيلون ماسك:** الملياردير والمبتكر.
            ...
            2. **التعلم التعاوني (الصديق الموازي):** الل
