        ...
        if st.button("✨ ابدأ التحليل والاستخراج", use_container_width=True):
            with st.spinner("الذكاء الاصطناعي يقوم بالتحليل..."):
                try:
                    ...
                    # تم تصحيح اسم النموذج هنا ليعمل مع الحساب المرقى بدون أخطاء ومشاكل انتهاء الصلاحية
                    ...
                    # تم تعديل رسالة الخطأ هنا لتظهر التفاصيل الدقيقة مباشرة في حال وجود أي مشكلة تقنية أخرى
                    st.error(f"حدث خطأ أثناء المعالجة. تفاصيل المشكلة: {str(e)}")
