...
with tab1:
    ...
    additional_notes = st.text_area("✍️ ملاحظات المعلم الإضافية:")
    uploaded_file = st.file_uploader("📸 حدد صورة الدرس:", type=["jpg", "png", "jpeg"])
    ...
    if st.session_state.analysis_done:
        st.markdown("---")
        st.write("النتيجة ستظهر هنا...")
        st.download_button("📥 تحميل Word", "بيانات", "plan.doc")
        st.download_button("📥 تحميل PDF", "بيانات", "plan.pdf")
        st.download_button("📥 تحميل صوت MP3", "بيانات", "plan.mp3")

# --- باقي التبويبات الأصلية ---
with tab2: st.markdown("### 🎯 أهداف المنصة")
with tab3: st.markdown("### ⚖️ قانون الدمج")
with tab4: st.markdown("### 👥 عن المنصة")
with tab5: st.markdown("### 📚 مكتبة الإعاقات والمقالات")
